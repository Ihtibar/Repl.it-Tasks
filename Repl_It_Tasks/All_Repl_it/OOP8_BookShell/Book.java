...
public class Book {
   ...
   String[] ret= word.split(" ");
		...
		word= word.substring(0,1).toUpperCase()+word.substring(1).toLowerCase();
    		...
    		word= ret[0].substring(0,1).toUpperCase()+ret[0].substring(1).toLowerCase();
    		for(int a=1; a<ret.length; a++) {
    			word+=" "+ret[a].substring(0,1).toUpperCase()+ret[a].substring(1).toLowerCase();
    ...
    public static boolean isTitleWord(String word) {
		      String[] myarr1= new String[]{"a" , "an", "the","and", "but", "for", "nor", "or", "so", "yet",
								 "at", "by", "in", "into", "near", "of", "on", "to", "than", "via"};
		String[] myarr2= word.split(" ");
		for(String mystr1:myarr1) 
			for(String mystr2 : myarr2) 
				if(mystr2.equalsIgnoreCase(mystr1)) 
				    return false;
    ...
    public Book(int id, String author, String title, int pages ) {
      setId(id);    setAuthor(author);
       setTitle(title);    setPages(pages);
    }
    ...
    public void setTitle(String title) {
      String[] arr= title.split(" ");
         if(title.length()==0)    this.title="";
        
         else if(arr.length==1) 
        	 this.title=capitalize(title);
         else {
        	 if(isTitleWord(title)) {
        		 this.title= capitalize(arr[0]);
        		
        		 for(int a=1; a<arr.length;a++) 
        			 this.title+=" "+capitalize(arr[a]);
        		 
        	 }else {
        		 this.title= capitalize(arr[0]);
        		 
        		 for(int a=1; a<arr.length;a++) 
        			 if(isTitleWord(arr[a])) 
        				 this.title+=" "+capitalize(arr[a]);
        			 else 
        				 this.title+=" "+arr[a].toLowerCase();
        	 }
         }
   }
